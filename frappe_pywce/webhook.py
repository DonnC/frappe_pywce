...
from frappe_pywce.util import CACHE_KEY_PREFIX, LIVE_MODE_WHATSAPP_ALERT_PREFIX, LOCK_WAIT_TIME, LOCK_LEASE_TIME, bot_settings, create_cache_key
from frappe_pywce.pywce_logger import app_logger as logger
from frappe_pywce.live_mode import get_live_context, get_message_for_live_mode, live_mode_active, send_wa_message
...
def _internal_webhook_handler(wa_id:str, payload:dict):
    ...
    try:
        ...
        with frappe.cache().lock(lock_key, timeout=LOCK_LEASE_TIME, blocking_timeout=LOCK_WAIT_TIME):
            if live_mode_active(wa_id):
                user_hook_arg = get_message_for_live_mode(payload)
                
                if not user_hook_arg: return

                custom_handler = frappe.get_hooks("pywce_live_inbound_handler")

                # if exists, developer owns the chat, pass to developer custom handler
                if custom_handler:
                    try:
                        resp_template = frappe.call(custom_handler[0], user_hook_arg)
                        send_wa_message(wa_id, resp_template, user_hook_arg)
                    ...
                    ticket_id = get_live_context(wa_id).get("ticket_name")
                    if ticket_id and frappe.db.exists("WhatsApp Support Ticket", ticket_id):
                        ...
                        frappe.get_doc("WhatsApp Support Ticket", ticket_id).add_comment(
                            "Comment", text=f"{LIVE_MODE_WHATSAPP_ALERT_PREFIX} {message_text}"
                        )
